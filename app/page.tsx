...
export default function Page() { const [game,setGame]=useState<GameId|null>(null); const [started,setStarted]=useState(false); const select=(id:string)=>{setGame(id as GameId);setStarted(false)}; const home=()=>{setGame(null);setStarted(false)}; const back=()=>{setStarted(false)}; return <AppShell onHome={home} onBack={game?back:undefined} title={game?({balance:'밸런스 게임',crocodile:'악어 이빨 게임',random:'랜덤 질문',choseong:'초성게임'}[game]):undefined}>{!game?<Home onSelect={select}/>:!started?<GameIntro id={game} onStart={()=>setStarted(true)}/>:game==='balance'?<BalanceGame/>:game==='crocodile'?<CrocodileGame/>:game==='random'?<RandomQuestionGame/>:<ChoseongGame/>}</AppShell>; }
